...
- Regenerated `spec/manifest.schema.json`: optional `label` on the shapes that UDL 4.6.0 labels, which beta.8 shipped without.
...
- Republished so the package cites a platform commit that survives the 2026-09-19 history rewrite. No vocabulary or conformance change.
